...
move_uploaded_file($_FILES["pic"]["tmp_name"], $target_dir.$target_file); 
  ...
  //$pic = trim(mysqli_real_escape_string($conn, $pic));
  ...
  if(isset($name)){
	  if($target_file != '')
	  {
		  $sql = "update  `product` set `name`='$name',`hsn`='$hsn',`unit`='$unit',`gst`='$gst',`descp`='$descp',`pic`='$target_file' where  id='$id'" ;
      }
	  else
	  {
		  $sql = "update  `product` set `name`='$name',`hsn`='$hsn',`unit`='$unit',`gst`='$gst',`descp`='$descp' where  id='$id'" ;
