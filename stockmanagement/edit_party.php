...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
 ...
 extract($_POST);
  ...
  if(isset($name)){
		  ...
		  $sql = "update  `party` set `cat`='$cat',`name`='$name',`addrress`='$addrress',`phn1`='$phn1',`phn2`='$phn2',
		  `gst`='$gst',`mailid`='$mailid',`date`='$date',`blnce`='$blnce' where  id='$id'" ;
     
    $res = mysqli_query($conn, $sql);
    if($res){
     $_SESSION['success_msg'] = "Party Updated successfull.";
